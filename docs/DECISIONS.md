...
2026-06-11 — Legacy token references are migrated with temporary saved-content compatibility — Older Ollie-derived templates, patterns, and CSS referenced undefined tokens such as `primary`, `main-accent`, `primary-accent`, `primary-alt`, `border-dark`, font family `primary`, font size `medium`, and spacing `xx-large`, while current theme tokens use the ICTS palette and spacing scale — Source markup now maps those references to current tokens, while `style.css` temporarily defines legacy variables/classes for already-saved database content until content can be audited and resaved.

2026-06-11 — Front-end utility scripts load from rendered markup instead of globally — Navigation, reveal, search modal, language switcher, page wires, and recovered counter fallback assets were being loaded broadly even when the matching UI was absent — The main stylesheet remains global, but utility scripts and static counter/reveal assets are now gated by saved content or rendered block/header markup, with ACF slider blocks enqueueing Flickity CSS alongside their slider JS.

2026-06-11 — Archive filter REST responses expose structured post data for DOM rendering — The archive filter endpoint previously returned escaped HTML that the browser injected with `innerHTML`, which was functional but fragile for future maintenance — The REST route now declares argument schemas with explicit sanitization/validation, returns structured post and pagination data, and the front-end renderer builds cards and pagination with DOM APIs while leaving legacy HTML fields temporarily available for compatibility.

2026-06-11 — Custom font-size entry is disabled in theme.json — The theme has the required font-size presets (`x-small`, `small`, `base`, `large`, `x-large`, `button`, and `h-1` through `h-6`) and the project prefers token-based editor controls — Editors can continue choosing preset sizes, while new arbitrary font-size values are removed from the UI to keep future output aligned with design tokens.

2026-06-11 — Hero content panel is rendered before JavaScript and homepage fonts are preloaded — Mobile testing showed the hero panel moving diagonally during load because the panel wrapper was being created after DOM ready, with additional movement from delayed Nunito Sans rendering — Hero Slide edit/save output now includes `.icts-hero-slider__content-panel`, existing saved slides are wrapped at render time for compatibility, and the front page preloads the local Nunito Sans regular/500/600 font files used by the hero.

2026-06-11 — Page wires load only for rendered wires containers and skip front-end editor observers — Decorative JS-generated wires add paint and observer work, but the homepage still requires the visual wires treatment — The front end now enqueues `assets/js/page-wires.js` only when rendered block markup contains the exact `page-wires-bg` class, and normal front-end execution skips editor-oriented mutation/iframe observers.
...
2026-04-17 — Testimonials Slider now supports block-level Customer Type filtering — Editors need to reuse the same testimonial slider across pages while narrowing results to a specific customer segment without duplicating testimonials — Added `customer-type` support to the `testimonial` CPT, introduced ACF block controls for `acf/testimonials-slider`, and updated the slider query to apply an optional `customer-type` tax query while preserving the existing Polylang and meta-based filtering behavior.

2026-04-17 — FAQ Accordion now supports an initial item limit with in-place reveal, and FAQ admin lists can be filtered by taxonomy — Editors need a lighter initial FAQ footprint on page plus a simple way to isolate FAQ subsets before reordering them with Post Types Order — Added block-level FAQ count and Show all label controls, front-end logic that limits initially visible matching FAQs until visitors reveal the rest, and admin dropdown filters for `product` and `customer-type` on the FAQ list screen.

2026-04-17 — FAQ ordering now supports filtered admin list reordering without relying on plugin drag handles in filtered archive views — Post Types Order disables drag-and-drop on taxonomy-filtered admin archives, but editors need to reorder FAQ subsets by product/customer type without losing global list stability — Added FAQ taxonomy filters plus custom drag-and-drop save logic on the FAQ admin list so filtered subsets can be reordered while the full published FAQ `menu_order` sequence remains stable outside the current subset.
...
2026-03-27 — Feature card internals favour editor-safe flow spacing — Editors can add arbitrary blocks inside cards, so positional selectors and rigid title/body assumptions were too brittle — `.feature-card__content` uses a simple vertical flex flow with consistent gap and bottom-pinned buttons, while title sizing is handled by explicit `.feature-card__title` rules and optional grid modifiers.
...
2026-04-29 — Two-line feature-card title alignment uses an explicit grid modifier — A global taller title min-height fixed wrapped titles but created excessive title/body spacing on grids where all titles fit on one line — Keep `feature-card__title` compact by default and add `feature-card-grid--title-2-lines` to the parent Grid block only when a card set needs two-line title equalisation.
