...
- When there are only a small number of logos, the block duplicates the logo set automatically to ensure even spacing, no visual gaps, and a smooth loop.
- This avoids a separate static slider mode and keeps the implementation lightweight and predictable.

Editor preview behaviour
- In the WordPress block editor, the slider renders as a static, centred preview with no animation, a single horizontal row, and a limited number of logos shown for clarity.
- This improves editor UX while avoiding unnecessary JS execution in the editor.
...
- This intentionally avoids issues caused by ACF field key drift between environments, such as Local JSON vs database state.
- Meta keys used: `testimonial_name`, `job_title`, `company`, `testimonial_text`, `client_logo`.
...
- Advanced Custom Fields Pro, including Options Pages and Local JSON.
- Polylang Pro is required for multilingual production workflows.
- WooCommerce is optional; WooCommerce-specific styles are only enqueued when WooCommerce is active.
- Flickity v2.x is vendored in the theme.
- Flickity CSS: `assets/vendor/flickity/flickity.min.css`
- Flickity JS: `assets/vendor/flickity/flickity.pkgd.min.js`
